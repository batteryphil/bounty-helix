...
class ToolExecutor:
    ...
    def _populate_registry(self):
        ...
        github_handlers = {
            ...
            "ws_run": lambda a: __import__("tools.workspace", fromlist=["ws_run"]).ws_run(a.get("slug",""), a.get("command",""), a.get("timeout",60)),
            "ws_write": lambda a: __import__("tools.workspace", fromlist=["ws_write"]).ws_write(a.get("slug",""), a.get("filepath",""), a.get("content","")),
            "ws_diff": lambda a: __import__("tools.workspace", fromlist=["ws_diff"]).ws_diff(a.get("slug","")),
            "ws_install_deps": lambda a: __import__("tools.workspace", fromlist=["ws_install_deps"]).ws_install_deps(a.get("slug","")),
